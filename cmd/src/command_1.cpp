...
int main(int argc, char **argv)
{
	...
	ros::Publisher pub_cmd = nh.advertise<geometry_msgs::Twist>("/a/cmd_vel_1", 1);
	ros::Publisher pub_goal = nh.advertise<std_msgs::Int32>("/goal_1", 1);
	...
	ros::Subscriber sub_odom = nh.subscribe("/a/odom_1", 1, Callback_odom);
	ros::Subscriber sub_imu = nh.subscribe("/a/imu_1", 1, Callback_imu);
	ros::Subscriber sub_yaw = nh.subscribe("/a/yaw_1", 1, Callback_yaw);
	ros::Subscriber sub_map = nh.subscribe("/map_1", 1, Callback_map);
	ros::Subscriber sub_go = nh.subscribe("/go", 1, Callback_go);
	ros::Subscriber sub_ob = nh.subscribe("/a/object_1", 1,Callback_object);
