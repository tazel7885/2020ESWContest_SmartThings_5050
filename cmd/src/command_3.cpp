...
int main(int argc, char **argv)
{
	...
	ros::Publisher pub_cmd = nh.advertise<geometry_msgs::Twist>("/c/cmd_vel_3", 1);
	ros::Publisher pub_goal = nh.advertise<std_msgs::Int32>("/goal_3", 1);
	...
	ros::Subscriber sub_odom = nh.subscribe("/c/odom_3", 1, Callback_odom);
	ros::Subscriber sub_imu = nh.subscribe("/c/imu_3", 1, Callback_imu);
	ros::Subscriber sub_yaw = nh.subscribe("/c/yaw_3", 1, Callback_yaw);
	ros::Subscriber sub_map = nh.subscribe("/map_3", 1, Callback_map);
	ros::Subscriber sub_go = nh.subscribe("/go", 1, Callback_go);
	ros::Subscriber sub_ob = nh.subscribe("/c/object_3", 1,Callback_object);
